...
Function z2t_VerifyClientTracking(cid, tid, uah)
    ...
    If dbCommand Is Nothing Then
        Exit Function
    End If
	Session("DaveChecking") = "Inside z2t_VerifyClientTracking"
	...
	Session("DaveChecking") = "Inside z2t_VerifyClientTracking2"

    Set rsResult = dbCommand.Execute
	Session("DaveChecking") = Err.number
    If Err.number = 0 Then
        'Success
		Session("DaveChecking") = "Inside z2t_VerifyClientTracking3"
        tsqlResult = dbCommand.Parameters("@tsqlResult")
        If tsqlResult = 0 Then
            'Success
			Session("DaveChecking") = "Inside z2t_VerifyClientTracking4"
...
Function z2t_Login_v25(userName, password, clientToken, ipAddress, clientType, idSession)
    ...
    'Client Token
    Set objParam = dbCommand.CreateParameter("@clientToken", adVarChar, adParamInput)
	If IsEmpty(clientToken) Or (Len(clientToken) = 0) Then clientToken = "1"
	...
	response.Write("<br/>ActiveConnection: " & dbCommand.ActiveConnection)
	If dbCommand.ActiveConnection Is Nothing Then Exit Function
    'if ipAddress = "72.43.236.2" then
    '    for each op in dbCommand.Parameters
'	    Response.write op.value & "<br>"
'        next
'	response.end
 '   end if
...
'	response.Write("<br/> End of z2t_Login_v25 Execute Command: <br/>")
'	response.Write("exec z2t_Login_v25 '" & _
'        userName & "','" & _
'        password & "','" & _ 
'	    clientToken  & "','" & _ 
'	    ipAddress  & "','" & _ 
...
Function z2t_NotesLookUp_v25(zipCode, ByRef recordCount)
    ...
    Set rsResult = dbCommand.Execute
	'Session("DaveChecking") = "z2t_NotesLookUp_v25"
